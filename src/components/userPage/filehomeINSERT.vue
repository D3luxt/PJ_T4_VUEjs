      ...
      <div class="col-start-2">
        ...
        <form @submit.prevent="confirmUpdate">
          ...
          <div class="form-control">
            ...
            <textarea v-model="filehome_details" id="filehome_details" name="filehome_details" rows="10" cols="50"
              v-bind:required="true"></textarea>
          ...
          <div class="form-control">
            ...
            <input id="filehomeIMG" name="filehomeIMG" type="file" ref="fileInput" accept=".png,.jpg,.jpeg" multiple
...
export default {
  ...
  data() {
    return {
      ...
      filehome_3d_url: "",
      filehome_details: "",
      filehomeIMG: [], // Change the default value to an empty array to store multiple files
  ...
  methods: {
    ...
    onFileSelected(event) {
      this.filehomeIMG = Array.from(event.target.files); // Store the selected files in the array
      console.log('Selected Files:', this.filehomeIMG); // Add this line to check the selected files
    },

    async uploadFile() {
      try {
        console.log('Selected Files:', this.filehomeIMG);

        if (!this.filehomeIMG || this.filehomeIMG.length === 0) {
          console.error('No files selected.'); // Handle the case when no file is selected
          return;
        }

        const formData = new FormData();
        for (const file of this.filehomeIMG) {
          formData.append('filehomeIMG', file); // Use 'filehomeIMG' as the key to match the backend
          console.log('img :' +file);
        }
       
        formData.append('filehome_details', this.filehome_details);
        ...
        if (response.status === 201) {
          console.log('Filehome(s) inserted successfully.');
          this.$router.replace("/Account");
        } else {
          console.error('Failed to insert filehome(s).');
