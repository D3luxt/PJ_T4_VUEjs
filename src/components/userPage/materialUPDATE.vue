...
export default {
  ...
  methods: {
    confirmUpdate() {
            this.$swal.fire({
                title: 'ยืนยันการแก้ไขข้อมูลวัสดุ',
                text: "ยืนยันการแก้ไขข้อมูลวัสดุ",
                icon: 'warning',
    ...
    enforceConstraints() {
            if (this.mat_gradeB > this.mat_gradeA) {
                this.mat_gradeB = this.mat_gradeA;
            }

            if (this.mat_gradeC > this.mat_gradeB) {
                this.mat_gradeC = this.mat_gradeB;
            }

            if (this.mat_gradeC > this.mat_gradeA) {
                this.mat_gradeC = this.mat_gradeA;
            }
        },
    async fetchMaterial(mat_id) {
      try {
        const response = await axios.get(`http://localhost:3001/material/${mat_id}`);
    ...
    async uploadFilehome() {
      try {
        ...
        if (response.status === 201) {
          console.log('mat inserted successfully.');
        } else {
    ...
    async updateFilehome() {
      try {
        const updatedData = {
          ...
          mat_gradeB: this.mat_gradeB,
          mat_gradeC: this.mat_gradeC,
        };

        if (this.materialIdToUpdate) { // Corrected variable name
          ...
          if (response.status === 200) {
            ...
            this.fetchMaterial(this.materialIdToUpdate);
          } else {
  ...
  mounted() {
    const materialIdToUpdate = this.$route.params.material_id; // Corrected parameter name
    if (materialIdToUpdate) {
      this.isUpdate = true;
      this.materialIdToUpdate = materialIdToUpdate; // Corrected variable name
      this.fetchMaterial(materialIdToUpdate);
    }
  },
  watch: {
        // Watch for changes in mat_gradeA and call enforceConstraints method
        mat_gradeA(newValue, oldValue) {
            this.enforceConstraints();
        },
        // Watch for changes in mat_gradeB and call enforceConstraints method
        mat_gradeB(newValue, oldValue) {
            this.enforceConstraints();
        },
        // Watch for changes in mat_gradeC and call enforceConstraints method
        mat_gradeC(newValue, oldValue) {
            this.enforceConstraints();
        },
    },
};
