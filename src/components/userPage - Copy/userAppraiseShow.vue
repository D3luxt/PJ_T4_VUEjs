    ...
    <div class="table-container">
        <table class="table-fixed">
            <thead>
                <tr>
                    <th>ID</th>
                    <th>รหัสการว่าจ้าง</th>
                    <th>เอกสาร</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item in filteredAppraiseData" :key="item.ap_id">
                    <td>{{ item.ap_id }}</td>
                    <td>{{ item.hire_id }}</td>
                    <td>
...
export default {
    computed: {
        ...
        filteredAppraiseData() {
            return this.appraise_data.filter(appraiseItem => {
                ...
                return matchingHire && this.currentUser.u_id === matchingHire.user_id ;
            });
        },
    },
    components: {},
    ...
    methods: {
        ...
        confirmAppraisal() {
            this.showConfirmation = false;
            this.updateHireStatus("รอทำสัญญา");
        },
        ...
        fetchhire_data() {
            axios
                .get('http://localhost:3001/hire') // Adjust the API endpoint according to your server
                .then(response => {
